...
def get_volatility():
    ...
    volatility, normalized_portfolio_values = Volatility.calculate_volatility(historical_data, allocations,
...
def get_var():
    ...
    historical_data = get_historical_data(coins, start_date, end_date)
    var, normalized_portfolio_values = Var.calculate_var(historical_data, allocations, initial_portfolio_value,
                                                         confidence_level)
...
@application.route('/sharpe_ratio1', methods=['POST'])
def get_sharpe_ratio1():
    try:
        ...
        graph = SharpeRatio1.create_graph_with_sharpe_ratio(portfolio_values, 'Portfolio Performance with Sharpe Ratio',
                                                            sharpe_ratio)
        return jsonify({'graph': graph})

    except Exception as e:
        application.logger.error('Error in Sharpe Ratio calculation: %s', str(e))
...
def get_sharpe_ratio():
    ...
    historical_data = get_historical_data(coins, start_date, end_date)
    sharpe_ratio = SharpeRatio.calculate_sharpe_ratio(historical_data, allocations, risk_free_rate,
                                                      initial_portfolio_value)
    graph = SharpeRatio.create_sharpe_ratio_graph(historical_data, allocations, initial_portfolio_value,
...
def get_annualized_returns():
    ...
    historical_data = get_historical_data(coins, start_date, end_date)
    annualized_returns, normalized_portfolio_values = AnnualizedReturns.calculate_annualized_returns(historical_data,
                                                                                                     allocations,
                                                                                                     initial_portfolio_value)
    graph = AnnualizedReturns.create_annualized_returns_graph(normalized_portfolio_values,
...
def get_expected_shortfall():
    ...
    historical_data = get_historical_data(coins, start_date, end_date)
    expected_shortfall, normalized_portfolio_values = ExpectedShortfall.calculate_expected_shortfall(historical_data,
                                                                                                     allocations,
                                                                                                     initial_portfolio_value)
    graph = ExpectedShortfall.create_expected_shortfall_graph(normalized_portfolio_values,
...
def get_historical_performance():
    ...
    historical_data = get_historical_data(coins, start_date, end_date)
    historical_performance = Performance.calculate_historical_performance(historical_data, allocations,
                                                                          initial_portfolio_value)
    graph = Performance.create_performance_graph(historical_performance,
...
def get_future_performance():
    ...
    historical_data = get_historical_data(coins, start_date, end_date)
    future_data = FuturePerformance.project_future_performance(historical_data, allocations, initial_portfolio_value,
                                                               days_in_future)
    graph = FuturePerformance.create_future_graph(future_data,
                                                  f'Projected Future Portfolio Value - Next {days_in_future} Days')

    return jsonify({'graph': graph})


@application.route('/future_performance_arima', methods=['POST'])
def get_future_performance_arima():
    ...
    future_data_scaled = future_data * initial_portfolio_value / future_data['Total'].iloc[
        0]  # Scale based on initial value
...
def get_drawdown_chart():
    try:
        ...
        graph = Drawdown.create_drawdown_chart(portfolio_drawdown, benchmark_drawdown,
                                               'Portfolio vs. Benchmark Drawdown')
        return jsonify({'graph': graph})

    except Exception as e:
        application.logger.error('Error occurred during drawdown chart creation: %s', str(e))
        return jsonify({'error': str(e)}), 500
